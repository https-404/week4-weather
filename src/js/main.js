...
import { OPENWEATHER_API_KEY, OPENWEATHER_GEOCODE_URL, DEFAULT_UNIT, } from "./config.js";
...
let currentUnit = localStorage.getItem("unit") || DEFAULT_UNIT;
...
async function fetchWeatherForecast(lat, lon, unit) {
...
function getWeekdayFromTimestamp(dt) {
    const date = new Date(dt * 1000);
    return date.toLocaleDateString(undefined, { weekday: "short" });
}
function transformForecastToOneCall(forecastData, cityName, country) {
    ...
    const daily = forecastData.daily.map((item, index) => ({
        dt: item.dt,
...
function setDynamicBackground(weatherMain) {
    ...
    const app = document.getElementById("app");
    const baseClasses = "min-h-screen flex items-center justify-center p-6 transition-colors duration-700";
...
async function loadWeather(city, unit) {
    try {
        ...
        const transformedData = transformForecastToOneCall(forecastData, name, country);
