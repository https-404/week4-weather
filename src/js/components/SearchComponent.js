export function createSearchElement(initialCity = "") {
    ...
    return {
        element: container,
        onSearch(cb) {
            button.onclick = () => cb(input.value.trim(), unitSelect.value);
            input.onkeypress = (e) => {
                if (e.key === "Enter")
                    cb(input.value.trim(), unitSelect.value);
            };
        },
        onUnitChange(cb) {
            unitSelect.onchange = () => cb(unitSelect.value);
        },
        onThemeToggle(cb) {
