import { OPENWEATHER_API_KEY, OPENWEATHER_GEOCODE_URL } from "./config.js";
...
export function isOneCallResponse(obj) {
...
export async function fetchWeatherForecast(lat, lon, unit = "metric") {
    const url = `https://api.openweathermap.org/data/2.5/forecast?lat=${lat}&lon=${lon}&units=${unit}&appid=${OPENWEATHER_API_KEY}`;
    const res = await fetch(url);
    if (!res.ok) {
        if (res.status === 401)
            throw new APIError("API key invalid (401)", 401);
        if (res.status === 429)
            throw new APIError("Rate limit exceeded (429)", 429);
